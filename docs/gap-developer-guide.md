...
Если апстрим не репортит `max_model_len` (например, Anthropic), оператор может
задать его вручную через `model_context_length` в конфиге провайдера —
конфиг всегда побеждает над значением, которое сообщил апстрим, и это же
значение попадёт клиентам в `/v1/models`.
...
### Лимит токенов на чанк

У эмбеддинг-моделей обычно есть жёсткий лимит на длину входа (512, 2048, 8192
токенов — зависит от модели). Прокси **не токенизирует** вход сам — это
требовало бы токенизатора под каждую модель, чего у нас нет. Вместо этого:

- лимит виден клиенту в поле `max_model_len` того же `/v1/models` (см. §2) —
  оператор может задать его вручную через `model_context_length` в конфиге
  провайдера, если апстрим сам его не репортит;
- если вы всё же отправите чанк длиннее лимита, апстрим вернёт свою ошибку
  (обычно `400`), и прокси пробросит её клиенту **как есть** — тот же код,
  то же сообщение, а не общий `502`. Это не специфично для эмбеддингов —
  `/v1/chat/completions` обрабатывает `4xx` от апстрима точно так же (см. §11).
...
Коды `400`/`401`/`404`/`413`/... в таблице выше — это ошибки **самого прокси**
(валидация до похода к апстриму: `type: "bad_request"`, `"model_not_found"` и
т.д.). Если же **все** кандидаты-провайдеры исчерпаны и последний из них
ответил своим `4xx` (апстрим отклонил сам запрос — например, слишком длинный
вход для `/v1/chat/completions` или `/v1/embeddings`), прокси пробрасывает
этот код и тело клиенту как есть, с `type: "provider_error"` — тот же код,
что и в таблице, но другая причина и другой `type`. `5xx` от апстрима всегда
схлопывается в `502` — это инфраструктурная проблема, а не проблема запроса
клиента.
