...
присутствует, прокси форвардит её как есть и не трогает `response_format`
(при этом `response_format`, если он тоже присутствует, из тела вырезается —
`grammar` имеет приоритет).
