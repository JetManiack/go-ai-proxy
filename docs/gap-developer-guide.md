...
| `api_key` | передавайте любую непустую строку — прокси сам по умолчанию её не проверяет (см. §9). Значение прозрачно отправляется апстриму как `Authorization: Bearer <key>` для тех провайдеров, что требуют ключ. |
...
прокси может пристрелить первую токен-задержку (см. §9).
...
## 12. Отладка: пустой ответ или «перемешанный» текст
